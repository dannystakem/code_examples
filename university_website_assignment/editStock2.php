...
<!-- Content Area for PHP Table -->
			...
			<?php
				...
				echo"<p>Please select which product you want to edit</p>";
				echo "<form action=\"editStock3.php\" method=\"post\">
				...
				// while loop to print out the table
				while ($row = mysqli_fetch_array($result)) {
					
					/*	
					 * the id variable is used to assign the value of the 
					 * items primary key to the radio button below.
					 */
					echo "<tr><td>{$row['ID']}</td>";
					$id = $row['ID'];
					echo "<td>{$row['name']}</td>";
					echo "<td><input type=\"radio\" name=\"selectProduct\" value=\"$id\"></td></tr>";
				}
				echo "</table><br><br>";
