...
def addLogRetention(retention_length):
    ...
    while True :
        log_group_list = log_group_list + [ x['logGroupName'] for x in response['logGroups'] ]
    ...
    for group in log_group_list:
        ...
        response = client.put_retention_policy(logGroupName=group, retentionInDays=retention_length)

    print('\nUpdated {} log groups - Added log retention of {} days'.format(len(log_group_list),retention_length))
